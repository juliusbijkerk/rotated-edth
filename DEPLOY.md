...
## Notes
- The always-up page is fully static; the live app runs on-demand behind the tunnel, so
  `app.rotated.cc` is only reachable while the laptop is serving it.
- A fully self-serve, always-on deployment would mean cloud-hosting the app and swapping
  local Whisper for a hosted STT API — out of scope for the static-landing showcase.
